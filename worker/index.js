/* SyncOps web demo proxy — Cloudflare Worker.
 ...
 *  3) Identity: always SyncOps, never reveal Gemini/Google, never leak keys/errors
 * No npm deps. Logs + uploads go to Backblaze B2 (native API), counters in KV if bound.
 */

const LIMIT_MSG =
  "That's your 2 free web replies — download SyncOps for unlimited voice, screen control, files and programs on your own PC.";
const FIREWALL_MSG =
  "That one's too big for this tiny web demo (over ~200 words / code). Open it in SyncOps on your PC where there's no limit — download SyncOps to run it fully.";
const BUSY_MSG = "SyncOps demo is busy right now — try again in a minute, or download SyncOps to run it locally.";
const GREET = "Hi, I'm SyncOps demo. Ask me something small — after 2 replies I'll point you to the desktop app.";

const SYSTEM =
  "You are SyncOps web demo, a tiny teaser of the SyncOps Windows desktop assistant. " +
  "Rules, never break: 1) You are SyncOps, running on the user's PC. Never say Gemini, Google, Bard, GPT, OpenAI, Anthropic, LLM, model, API, key, proxy, worker, or prompt. " +
  "If asked what powers you, say: 'I am SyncOps running on your PC.' 2) Keep every answer UNDER 120 words, plain text, no code blocks unless the question is trivially small (max 10 lines). " +
...
function scrub(s) {
  return String(s || "").replace(PROVIDER_LEAK_RE, "SyncOps").slice(0, 1200);
}
function upsell(extra) {
  return { reply: extra || FIREWALL_MSG, blocked: "firewall", left: 0, by: "SyncOps" };
}
...
function logB2(env, ctx, rec) {
  try {
    ...
    ctx.waitUntil(b2Upload(env, name, JSON.stringify({ ...rec, by: "SyncOps", at: new Date().toISOString() })));
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && url.pathname.endsWith("/api/status")) {
      return json({ ok: true, demo: "SyncOps", free: 2, by: "SyncOps" }, 200, h);
    }
    ...
    if (request.method === "POST" && url.pathname.endsWith("/api/upload")) {
      try {
        ...
        if (!f || typeof f === "string") return json({ reply: FIREWALL_MSG, blocked: "firewall", by: "SyncOps" }, 200, h);
        if (f.size > 5 * 1024 * 1024) return json({ reply: FIREWALL_MSG, blocked: "firewall", by: "SyncOps" }, 200, h);
        ...
        return json({ reply: "Got “" + String(f.name).slice(0, 60) + "” in this demo — open it in SyncOps on your PC to process it fully.", blocked: "firewall", by: "SyncOps" }, 200, h);
      } catch {
        return json({ reply: BUSY_MSG, by: "SyncOps" }, 200, h);
      }
    }

    if (request.method === "POST" && url.pathname.endsWith("/api/chat")) {
      let body = {};
      try { body = await request.json(); } catch { return json({ reply: BUSY_MSG, by: "SyncOps" }, 200, h); }
      ...
      if (!message) return json({ reply: GREET, left: 2, by: "SyncOps" }, 200, h);
      ...
      if (a >= 2 || b >= 6) {
        ...
        return json({ reply: LIMIT_MSG, blocked: "limit", left: 0, by: "SyncOps" }, 200, h);
      }
      ...
      if (!env.GEMINI_API_KEY) return json({ reply: BUSY_MSG, by: "SyncOps" }, 200, h);
      try {
        ...
        return json({ reply: out, left: Math.max(0, left - 1), by: "SyncOps" }, 200, h);
      } catch {
        return json({ reply: BUSY_MSG, by: "SyncOps" }, 200, h);
      }
    }
    return json({ ok: true, demo: "SyncOps", by: "SyncOps" }, 200, h);
  },
};
